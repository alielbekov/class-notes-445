...
Approach(recursive): If the shortest path has a solution that goes from Node I to Node J(m edges), then we can remove the last node and create another shortest path from I to H (m-1 edges).
